...
struct scheduler {
 public:
  ...
  scheduler(size_t _num_threads = 0)
      : num_threads(_num_threads == 0 ? init_num_workers() : _num_threads),
...
class fork_join_scheduler {
 ...
 public:
  ...
  void set_num_workers(size_t n) {
    assert(n <= std::max<size_t>(16, std::thread::hardware_concurrency()));
    sched = std::make_unique<scheduler<Job>>(n);
  }
